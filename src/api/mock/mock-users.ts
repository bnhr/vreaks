import { MOCK_API_CONFIG } from '~/config/api-config'
import { loadMockData, saveMockData } from './mock-data'
import type {
	UserListResponse,
	UserResponse,
	UserPayload,
...
export async function mockUpdateUser(
	...
	const emailTaken =
		payload.email &&
		data.users.some((u) => u.email === payload.email && u.id !== userId)
	const usernameTaken =
		payload.username &&
		data.users.some((u) => u.username === payload.username && u.id !== userId)
